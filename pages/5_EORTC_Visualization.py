...
@st.cache_data
def load_data():
    file_path = f"{folder_stem}eortc/eortc_actual_scores.csv"  # Adjust the path
...
total_possible_scores = df.shape[0]/17


# Title
st.title("🏥 Overall Hospital EORTC Status")
...
col2.metric("Avg EORTC Score (Before)", round(filtered_df["final_score_1"].mean(), 2))
col3.metric("Avg EORTC Score (After)", round(filtered_df["final_score_2"].mean(), 2))
...
#====================Missing EORTC Scores by Dimension====================

# Grouped Bar Chart with Threshold Line
st.subheader("📊 Missing EORTC Scores by Dimension")
...
filtered_total_possible_scores = filtered_df.shape[0] / 17
...
ax.set_xlabel("EORTC Dimension")
ax.set_title("Missing EORTC Scores Before & After (Filtered)")
...
st.subheader("📌 Average EORTC Scores by Department")
...
ax.set_ylabel("Average EORTC Score")
ax.set_xlabel("Department")
ax.set_title("Department-wise EORTC Score Averages")
...
#====================Mean EORTC Scores by Dimension (Before & After)====================


# Compute mean EORTC scores for each dimension using filtered_df
...
st.subheader("📊 Mean EORTC Scores by Dimension (Before & After)")
...
# Formatting
ax.set_ylabel("Mean EORTC Score")
ax.set_xlabel("EORTC Dimension")
ax.set_title("Mean EORTC Scores Across Dimensions (Before & After)")
...
#====================Insights & Alerts - EORTC Scores above threshold ====================
...
criteria = []
if (filtered_df["final_score_1"] >= 3).any():
    criteria.append("Final Score Before (≥4)")
if (filtered_df["final_score_2"] >= 3).any():
    criteria.append("Final Score After (≥4)")

# Apply filtering logic
high_severity_cases = filtered_df[(filtered_df["final_score_1"] >= 3) | (filtered_df["final_score_2"] >= 3)]
